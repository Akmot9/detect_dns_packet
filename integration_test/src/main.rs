...
fn handle_udp_packet(interface_name: &str, source: IpAddr, destination: IpAddr, packet: &[u8]) {
    ...
    if let Some(udp) = udp {
        ...
        let l7_packet = udp.payload();
        match DnsPacket::try_from(l7_packet) {
            Ok(dns) => {
                println!("{:?}", dns);
            }
            Err(e) => {
                println!("[{}]: Unknown UDP Packet", interface_name);
            }
        }
...
fn handle_tcp_packet(interface_name: &str, source: IpAddr, destination: IpAddr, packet: &[u8]) {
    ...
    if let Some(tcp) = tcp {
        ...
        let l7_packet = tcp.payload();
        let dns = DnsPacket::try_from(l7_packet);
        println!("{:?}", dns);
    } else {
