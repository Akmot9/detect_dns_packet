...
impl fmt::Display for DnsQuery {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "DnsQuery {{ name: {}, qtype: {}, qclass: {} }}", self.name, self.qtype, self.qclass)
    }
}
...
mod tests {
    ...
    #[test]
    fn test_parse_name() {
        let data = vec![0x03, 0x77, 0x77, 0x77, // "www"
    ...
    fn test_dns_query_from_bytes() {
        let data = vec![3, b'w', b'w', b'w', 6, b'g', b'o', b'o', b'g', b'l', b'e', 3, b'c', b'o', b'm', 0, 0, 1, 0, 1];
        let mut offset = 0;
    ...
    fn test_dns_queries_from_bytes() {
        let data = vec![
            3, b'w', b'w', b'w', 6, b'g', b'o', b'o', b'g', b'l', b'e', 3, b'c', b'o', b'm', 0, 0, 1, 0, 1,
            3, b'f', b'o', b'o', 3, b'b', b'a', b'r', 3, b'c', b'o', b'm', 0, 0, 2, 0, 1
        ];
        ...
        assert_eq!(queries.queries[0].qtype, 1);
        assert_eq!(queries.queries[0].qclass, 1);
        assert_eq!(queries.queries[1].name, "foo.bar.com");
        assert_eq!(queries.queries[1].qtype, 2);
        assert_eq!(queries.queries[1].qclass, 1);
    }
}
